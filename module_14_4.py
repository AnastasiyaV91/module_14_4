...
async def get_buying_list(message):
    await message.answer(crud_functions.get_all_products(1))  # Вывод сообщения о продукте
    with open("1.jpg", "rb") as img1:                         # Открытие фотографии продукта
        await message.answer_photo(img1)                      # Вывод фотографии продукта
    await message.answer(crud_functions.get_all_products(2))
    with open("2.jpg", "rb") as img2:
        await message.answer_photo(img2)
    await message.answer(crud_functions.get_all_products(3))
    with open("3.jpg", "rb") as img3:
        await message.answer_photo(img3)
    await message.answer(crud_functions.get_all_products(4))
